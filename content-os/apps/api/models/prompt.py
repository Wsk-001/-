...
class Prompt(Base):
    ...
    owner_id: Mapped[Optional[uuid.UUID]] = mapped_column(GUID(), ForeignKey("users.id"), nullable=True)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    category: Mapped[str] = mapped_column(String(50), nullable=False)
    current_version_id: Mapped[Optional[uuid.UUID]] = mapped_column(GUID(), ForeignKey("prompt_versions.id"), nullable=True)
...
class PromptVersion(Base):
    ...
    model_hint: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
